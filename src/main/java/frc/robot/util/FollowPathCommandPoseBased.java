...
/** Base command for following a path. (Modified so it doesn't stop until we reach our target) */
public class FollowPathCommandPoseBased extends Command {
  ...
  private final double translation_deadzone;
  private final double rotation_deadzone;
   ...
   * @param translation_deadzone The maximum distance from the target to end the path, in meters.
   * @param rotation_deadzone The maximum rotational error from the target to end the path, in
   *     degrees.
   * @param requirements Subsystems required by this command, usually just the drive subsystem
   */
  public FollowPathCommandPoseBased(
    ...
    this.translation_deadzone = translation_deadzone;
  ...
  public boolean isFinished() {
    Pose2d currentPose = poseSupplier.get();
    Pose2d targetPose = trajectory.getEndState().pose;

    boolean timer_finished = timer.hasElapsed(trajectory.getTotalTimeSeconds());
    boolean translate_acceptable =
        currentPose.getTranslation().getDistance(targetPose.getTranslation())
            < translation_deadzone;
    boolean rotation_acceptable =
        Math.abs(currentPose.getRotation().minus(targetPose.getRotation()).getDegrees())
            < rotation_deadzone;

    return timer_finished && translate_acceptable && rotation_acceptable;
  }
