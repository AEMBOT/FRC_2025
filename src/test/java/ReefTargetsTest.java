...
public final class ReefTargetsTest {
  ...
  void testTarget() {
    ...
    try{
      assertEquals(8,testCase.findClosestReef(new Pose2d(5.5,8,new Rotation2d(0))),"target = 8");
    } catch (AssertionError e){
      System.out.println(e.getMessage());
    }

    try{
      assertEquals(9,testCase.findClosestReef(new Pose2d(3.5,8,new Rotation2d(0))),"target = 9");
    } catch (AssertionError e){
      System.out.println(e.getMessage());
    }

    try{
      assertEquals(10,testCase.findClosestReef(new Pose2d(2,7,new Rotation2d(0))),"target = 10");
    } catch (AssertionError e){
      System.out.println(e.getMessage());
    }

    try{
      assertEquals(11,testCase.findClosestReef(new Pose2d(1,5,new Rotation2d(0))),"target = 11");
    } catch (AssertionError e){
      System.out.println(e.getMessage());
    }
